...
class NumaAwareCommunityHypergraphPruner final : public HypergraphPrunerBase<Hypergraph, 
                                                                             kahypar::ds::NumaAwareCommunityHypergraph<Hypergraph>, 
                                                                             NumaAwareCommunityHypergraphPruner> {
  using NumaAwareCommunityHypergraph = kahypar::ds::NumaAwareCommunityHypergraph<Hypergraph>;
  using Base = HypergraphPrunerBase<Hypergraph, NumaAwareCommunityHypergraph, NumaAwareCommunityHypergraphPruner>;
  friend Base;
 ...
 public:
  explicit NumaAwareCommunityHypergraphPruner(const HypernodeID max_num_nodes, 
                                              const PartitionID community_id) :
    Base(max_num_nodes, community_id) { }

  NumaAwareCommunityHypergraphPruner(const NumaAwareCommunityHypergraphPruner&) = delete;
  NumaAwareCommunityHypergraphPruner& operator= (const NumaAwareCommunityHypergraphPruner&) = delete;

  NumaAwareCommunityHypergraphPruner(NumaAwareCommunityHypergraphPruner&&) = default;
  NumaAwareCommunityHypergraphPruner& operator= (NumaAwareCommunityHypergraphPruner&&) = default;

  virtual ~NumaAwareCommunityHypergraphPruner() = default;
  ...
  // parallel. In case we detect a parallel HE, it is removed from the graph and we proceed by
  // checking if there are more fingerprints with the same hash value.
  HyperedgeID removeParallelHyperedgesImpl(NumaAwareCommunityHypergraph& hypergraph,
                                           CoarseningMemento& memento) {
    memento.parallel_hes_begin = _removed_parallel_hyperedges.size();
