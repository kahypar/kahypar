...
hg = open(graph, 'r')
modified_hg_path = '/tmp/modified_' + str(os.getpid())+'_'+basename(graph) + '.hgr'
...
start = time.time()
p = Popen(['/software/patoh-Linux-x86_64/Linux-x86_64/patoh',
           modified_hg_path,
           str(k),
           'SD='+str(seed), # seed
           'FI='+str(ufactor/100.0), # imbalance ratio
           'OD=3', # non verbose output
           'UM=U', # net-cut metric
...
print(result_string + " type=PaToH-Quality" +  " measuredTotalPartitionTime=" + str(end-start))
...
# delete the modified hg
os.remove(modified_hg_path)
