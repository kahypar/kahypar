...
class NoDataBinaryMaxHeap{
 ...
 public:
  ...
  NoDataBinaryMaxHeap( const NoDataBinaryMaxHeap & other) :
  ...
  NoDataBinaryMaxHeap( const id_slot & storage_initializer)
  ...
  void swap(NoDataBinaryMaxHeap& other) {
    using std::swap;
  ...
  inline const id_slot & getMax() const{
    GUARANTEE( !empty(), std::runtime_error,
               "[error] BinaryHeap::getMin() - Requesting minimum of empty heap" )
    return heap[1].id;
  }

  inline const key_slot & getMaxKey() const {
    GUARANTEE( !empty(), std::runtime_error,
...
void swap(NoDataBinaryMaxHeap<id_slot,key_slot,meta_key_slot,storage_slot>& a,
          NoDataBinaryMaxHeap<id_slot,key_slot,meta_key_slot,storage_slot>& b) {
  a.swap(b);
}
