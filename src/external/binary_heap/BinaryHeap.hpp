...
class BinaryHeap{
 ...
 public:
  ...
  BinaryHeap( const BinaryHeap & other) :
      storage(other.storage.sizeE()),
      data_elements( new DataElement<id_slot, key_slot, data_slot>[other.storage.sizeE()] ){
    number_of_elements = 0;
    heap.reserve(other.heap.capacity());
    heap.push_back(HeapElement( meta_key_slot::min() ));
  }
