...
class Partitioner {
	...
	enum class RBHypergraphState
		: std::uint8_t {
			unpartitioned, partitionedAndPart1Extracted, finished
	};
...
private:
	FRIEND_TEST(APartitionerWithHyperedgeSizeThreshold, RemovesHyperedgesExceedingThreshold);FRIEND_TEST(APartitionerWithHyperedgeSizeThreshold, RestoresHyperedgesExceedingThreshold);FRIEND_TEST(APartitionerWithHyperedgeSizeThreshold,
			PartitionsUnpartitionedHypernodesAfterRestore);FRIEND_TEST(APartitionerWithHyperedgeSizeThreshold,
			AssignsAllRemainingHypernodesToDefinedPartition);FRIEND_TEST(APartitionerWithHyperedgeSizeThreshold,
			TriesToMinimizesCutIfNoPinOfRemainingHyperedgeIsPartitioned);FRIEND_TEST(APartitionerWithHyperedgeSizeThreshold,
			TriesToMinimizesCutIfOnlyOnePartitionIsUsed);FRIEND_TEST(APartitionerWithHyperedgeSizeThreshold,
			DistributesAllRemainingHypernodesToMinimizeImbalaceIfCutCannotBeMinimized);FRIEND_TEST(APartitioner, UseshMetisPartitioningOnCoarsestHypergraph);FRIEND_TEST(APartitioner, UncoarsensTheInitiallyPartitionedHypergraph);FRIEND_TEST(APartitioner, CalculatesPinCountsOfAHyperedgesAfterInitialPartitioning);FRIEND_TEST(APartitioner, CanUseVcyclesAsGlobalSearchStrategy);
	...
	inline void createMappingsForInitialPartitioning(
			HmetisToCoarsenedMapping& hmetis_to_hg,
	...
	inline bool partitionVCycle(Hypergraph& hypergraph, ICoarsener& coarsener,
			IRefiner& refiner, const Configuration& config, const int vcycle,
			const PartitionID k1, const PartitionID k2);
	inline HypernodeID originalHypernode(const HypernodeID hn,
			const MappingStack& mapping_stack) const;
	inline double calculateRelaxedEpsilon(
			const HypernodeWeight original_hypergraph_weight,
			const HypernodeWeight current_hypergraph_weight,
			const PartitionID k, const Configuration& original_config) const;

	inline Configuration createConfigurationForCurrentBisection(
...
inline void Partitioner::partition(Hypergraph& hypergraph,
		const Configuration& config) {
...
inline double Partitioner::calculateRelaxedEpsilon(
		const HypernodeWeight original_hypergraph_weight,
...
inline Configuration Partitioner::createConfigurationForCurrentBisection(
	...
	current_config.partition.perfect_balance_part_weights[0] =
			ceil(
					(k0 / static_cast<double>(current_k))
							* static_cast<double>(current_config.partition.total_graph_weight));

	current_config.partition.perfect_balance_part_weights[1] =
			ceil(
					(k1 / static_cast<double>(current_k))
							* static_cast<double>(current_config.partition.total_graph_weight));

	current_config.partition.max_part_weights[0] = (1
			+ current_config.partition.epsilon)
			* current_config.partition.perfect_balance_part_weights[0];

	current_config.partition.max_part_weights[1] = (1
			+ current_config.partition.epsilon)
			* current_config.partition.perfect_balance_part_weights[1];

	current_config.coarsening.contraction_limit =
			current_config.coarsening.contraction_limit_multiplier
...
inline void Partitioner::performRecursiveBisectionPartitioning(
		Hypergraph& input_hypergraph, const Configuration& original_config) {
	...
	while (!hypergraph_stack.empty()) {
		...
		if (hypergraph_stack.back().lower_k
				== hypergraph_stack.back().upper_k) {
			for (const HypernodeID hn : current_hypergraph.nodes()) {
				const HypernodeID original_hn = originalHypernode(hn,
						mapping_stack);
				const PartitionID current_part = input_hypergraph.partID(
						original_hn);
				ASSERT(current_part != Hypergraph::kInvalidPartition,
						V(current_part));
...
inline void Partitioner::performDirectKwayPartitioning(Hypergraph& hypergraph,
	...
	// TODO(schlag): find better solution
	_internals.append(
			coarsener->policyString() + " " + refiner->policyString());

	partition(hypergraph, *coarsener, *refiner, config, 0,
			(config.partition.k - 1));
	...
	for (int vcycle = 1; vcycle <= config.partition.global_search_iterations;
		...
		if (!found_improved_cut) {
			LOG(
					"Cut could not be decreased in v-cycle " << vcycle << ". Stopping global search.");
			break;
		}
...
inline void Partitioner::createMappingsForInitialPartitioning(
		HmetisToCoarsenedMapping& hmetis_to_hg,
		CoarsenedToHmetisMapping& hg_to_hmetis, const Hypergraph& hg) {
	int i = 0;
...
inline void Partitioner::removeLargeHyperedges(Hypergraph& hg,
		Hyperedges& removed_hyperedges, const Configuration& config) {
		...
		for (const HyperedgeID he : hg.edges()) {
			if (hg.edgeSize(he) > config.partition.hyperedge_size_threshold) {
				DBG(dbg_partition_large_he_removal,
						"Hyperedge " << he << ": size (" << hg.edgeSize(he) << ")   exceeds threshold: " << config.partition.hyperedge_size_threshold);
	...
	const HypernodeWeight max_part_weight = std::max(
	...
	if (config.partition.remove_hes_that_always_will_be_cut) {
		for (const HyperedgeID he : hg.edges()) {
			...
			if (sum_pin_weights > max_part_weight) {
				DBG(dbg_partition_large_he_removal,
						"Hyperedge " << he << ": w(pins) (" << sum_pin_weights << ")   exceeds Lmax: " << max_part_weight);
				removed_hyperedges.push_back(he);
				hg.removeEdge(he, false);
			}
		}
	...
	LOG(
			"removed " << removed_hyperedges.size() << " HEs that had more than " << config.partition.hyperedge_size_threshold << " pins or weight of pins was greater than Lmax=" << max_part_weight);
}

inline void Partitioner::restoreLargeHyperedges(Hypergraph& hg,
		const Hyperedges& removed_hyperedges) {
	for (auto && edge = removed_hyperedges.rbegin();
			edge != removed_hyperedges.rend(); ++edge) {
