...
TEST_F(InitialPartitionerBaseTest, ExtractingHypergraphFromPartition) {
	...
	partitioner->extractPartitionAsHypergraph(hypergraph, 0, num_hypernodes_0,
			num_hyperedges_0, index_vector_0, edge_vector_0,
			hyperedge_weights_0, hypernode_weights_0,
			hgToExtractedHypergraphMapper_0);
	Hypergraph partition_0(num_hypernodes_0, num_hyperedges_0, index_vector_0,
			edge_vector_0, config.initial_partitioning.k, &hyperedge_weights_0,
			&hypernode_weights_0);
	...
	std::vector<HypernodeID> test_node { 0, 1, 2, 3 };
	int i = 0;
	for (HypernodeID hn : partition_0.nodes()) {
		ASSERT_EQ(hgToExtractedHypergraphMapper_0[hn], test_node[i]);
		ASSERT_EQ(partition_0.nodeWeight(hn),
				hypergraph.nodeWeight(hgToExtractedHypergraphMapper_0[hn]));
		i++;
	}

	//Testing extract hyperedge
	std::vector<HypernodeID> test_node_in_edge { 0, 2 };
	i = 0;
	...
	partitioner->extractPartitionAsHypergraph(hypergraph, 1, num_hypernodes_1,
			num_hyperedges_1, index_vector_1, edge_vector_1,
			hyperedge_weights_1, hypernode_weights_1,
			hgToExtractedHypergraphMapper_1);
	Hypergraph partition_1(num_hypernodes_1, num_hyperedges_1, index_vector_1,
			edge_vector_1, config.initial_partitioning.k, &hyperedge_weights_1,
			&hypernode_weights_1);
	...
	//Testing hypernode mapping
	std::vector<HypernodeID> test_node_2 { 4, 5, 6 };
	i = 0;
	for (HypernodeID hn : partition_1.nodes()) {
		ASSERT_EQ(hgToExtractedHypergraphMapper_1[hn], test_node_2[i]);
		ASSERT_EQ(partition_1.nodeWeight(hn),
				hypergraph.nodeWeight(hgToExtractedHypergraphMapper_1[hn]));
		i++;
	}
