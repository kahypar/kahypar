	...
	if (vm.count("hgr") && vm.count("e") && vm.count("k")) {
		...
		if (vm.count("part-path")) {
			...
			case InitialPartitioner::KaHyPar:
				config.partition.initial_partitioner_path =
						"../partition/initial_partitioning/application/InitialPartitioningKaHyPar";
				break;
			}
		}
		if (vm.count("mode")) {
...
int main(int argc, char* argv[]) {
	...
	desc.add_options()("help", "show help message")("verbose",
			...
			po::value<int>(), "Seed for random number generator")(
			...
			"Initial Partitioner: hMetis (default), PaToH")("part-path",
			po::value<std::string>(), "Path to Initial Partitioner Binary")(
			"vcycles", po::value<int>(), "# v-cycle iterations")("cmaxnet",
			po::value<HyperedgeID>(),
			...
			"file", po::value<std::string>(), "filename of result file")("mode",
			po::value<std::string>(), "Initial partitioning mode");
	...
	io::readHypergraphFile(config.partition.graph_filename, num_hypernodes,
			num_hyperedges, index_vector, edge_vector);
	...
	for (const HyperedgeID he : hypergraph.edges()) {
		ASSERT(
				[&]() -> bool { HypernodeID num_pins = 0; for (PartitionID i = 0; i < config.partition.k; ++i) { num_pins += hypergraph.pinCountInPart(he, i); } return num_pins == hypergraph.edgeSize(he); } (),
