...
class KWayPriorityQueue2 {
#ifdef USE_BUCKET_PQ
  using Queue = EnhancedBucketQueue<IDType, KeyType, MetaKey>;
#else
  using Queue = BinaryMaxHeap<IDType, KeyType, MetaKey>;
#endif
 ...
 public:
  explicit KWayPriorityQueue2(const PartitionID k, Hypergraph& hg, const HypernodeWeight max_part_weight) noexcept :
    _queues(),
    ...
    _num_enabled_pqs(0),
    _hg(hg),
    _max_part_weight(max_part_weight) { }

  KWayPriorityQueue2(const KWayPriorityQueue2&) = delete;
  KWayPriorityQueue2& operator= (const KWayPriorityQueue2&) = delete;

  KWayPriorityQueue2(KWayPriorityQueue2&&) = default;
  KWayPriorityQueue2& operator= (KWayPriorityQueue2&&) = delete;
  ...
  __attribute__ ((always_inline)) void insert(const IDType id, const PartitionID part,
    ...
    if (_hg.partWeight(part) + _hg.nodeWeight(_queues[_mapping[part].index].top()) <= _max_part_weight) {
      enablePart(part);
    } else {
      disablePart(part);
    }
  ...
  __attribute__ ((always_inline)) void deleteMax(IDType& max_id, KeyType& max_key,
    ...
    ASSERT(_hg.partWeight(max_part) + _hg.nodeWeight(_queues[max_index].top()) <= _max_part_weight, V(max_part) <<
           V(max_id) << V(max_key));
    ...
    if (_queues[max_index].empty()) {
      ...
      if (_hg.partWeight(max_part) + _hg.nodeWeight(_queues[max_index].top()) <= _max_part_weight) {
        enablePart(max_part);
      } else {
        disablePart(max_part);
      }
    }
    --_num_entries;
  }
  ...
  void updatePQstates() {
    for (size_t index = 0; index < _num_enabled_pqs; ++index) {
      ...
      if (_hg.partWeight(part) + _hg.nodeWeight(_queues[index].top()) <= _max_part_weight) {
        enablePart(part);
      } else {
        disablePart(part);
      }
 ...
 private:
  ...
  __attribute__ ((always_inline)) size_t maxIndex() const noexcept {
    ...
    for (size_t index = 0; index < _num_enabled_pqs; ++index) {
      ...
      ASSERT(_mapping[_mapping[index].part].index == index, V(index));
      const KeyType key = _queues[index].topKey();
      ASSERT(_hg.partWeight(_mapping[index].part) + _hg.nodeWeight(_queues[index].top()) <= _max_part_weight,
             V(_mapping[index].part) << V(max_key) << V(key));
      if (key > max_key) {
        max_key = key;
        max_index = index;
      }
  ...
  Hypergraph& _hg;
  const HypernodeWeight _max_part_weight;
};
...
constexpr size_t KWayPriorityQueue2<IDType, KeyType, MetaKey, Storage,
...
template <typename IDType,
          ...
          class Storage,
          bool UseRandomTieBreaking>
constexpr PartitionID KWayPriorityQueue2<IDType, KeyType, MetaKey, Storage,
